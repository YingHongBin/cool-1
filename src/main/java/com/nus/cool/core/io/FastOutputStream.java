...
public class FastOutputStream extends OutputStream {
  ...
  /**
   * Creates a new bute array output stream. The buffer capacity in initially 32 bytes, though its
  ...
  /**
   * Writes the complete contents of this byte array output strean ti the specified output stream
   * argument, as if by calling the output stream's write method using <code>out.write(buffer, 0,
   * count)</code>.
   *
   ...
   *
   * <p>
   * This method always replaces malformed-input and unmappable-character sequences with the default
   ...
   *                          unsatisfiable large capacity {@code (long) Integer.MAX_VALUE +
