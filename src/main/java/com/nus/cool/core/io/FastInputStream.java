...
public class FastInputStream extends InputStream {
   ...
   * <code>buffer[0]</code> through <code>buffer[count-1]</code> are the only bytes that
   * can ever be read from the stream; element <code>buffer[pos]</code> is the next byte to be
   * read.
   */
   ...
   * position zero by default when constructed. They may be marked at another position within the
   ...
   * should always be nonnegative and not larger than the length of <code>buffer</code>. It is one
   * greater than the position of the last byte within <code>buffer</code> that can ever be read
   * from the input stream buffer.
   */
  protected int count;

  /**
   * Creates a <code>ByteArrayInputStream</code> so that it uses <code>buffer</code> as its buffer
  ...
  /**
   * Creates <code>ByteArrayInputStream</code> that uses <code>buffer</code> as its buffer array.
   * The initial value of <code>pos</code> is <code>offset</code> and the initial value of
   * <code>count</code> is the minimum of
   * <code>offset+length</code> and <code>buffer.length</code>. The buffer array is
  ...
  /**
   * Set the current marked position in the stream. ByteArrayInputStream objects are marked at
   * position zero by default when constructed. They may be marked at another position within the
  ...
  /**
   * Closing a <tt>ByteArrayInputStream</tt> has no effect. The methods in this class can be called
