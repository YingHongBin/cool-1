  ...
  public void process(
    ...
    for (Map.Entry<String, BitSet> entry : groups.entrySet()) {
      ...
      if (result.getCountDistinct() == null) {
        ...
        if (field.isPreCal()) {
          // if field has pre-calculate bitsets, execute and function between selection bitset and
          // field value bitset, if the result is not empty, then add the value to distinct set
